...
public class Calculator extends Panel{
	...
	private void buildUi(String title) {
		...
		clearContent = new ClearContent();
		clearContent.setClearContentListener(calculatorBrain);
		calculatorWindow.add(clearContent);
		
		Panel p = new Panel();
		p.setLayout(new GridLayout(2,1));
		p.add(displayPanel);
		p.add(clearContent);
		calculatorWindow.add(p,BorderLayout.NORTH);
